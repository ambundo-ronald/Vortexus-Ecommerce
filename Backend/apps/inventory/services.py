...
def _reservation_allocated_quantity(reservation) -> int:
    if not reservation or not getattr(reservation, 'allocation_applied', False):
        return 0
...
def sync_basket_line_reservation(line, desired_quantity: int | None = None):
    ...
    if reservation and (reservation.stockrecord_id != line.stockrecord_id or not _can_track_stock(stockrecord)):
        ...
        allocated_quantity = _reservation_allocated_quantity(reservation)
        if _can_track_stock(locked_stockrecord) and allocated_quantity:
            locked_stockrecord.cancel_allocation(allocated_quantity)
        reservation.delete()
        reservation = None
    ...
    allocated_quantity = _reservation_allocated_quantity(reservation)
    available_quantity = _available_for_line(locked_stockrecord, allocated_quantity)
    ...
    if reservation:
        if allocated_quantity:
            locked_stockrecord.cancel_allocation(allocated_quantity)
            reservation.allocation_applied = False
        if reservation.quantity != desired_quantity:
            reservation.quantity = desired_quantity
        reservation.save(update_fields=['quantity', 'allocation_applied', 'updated_at'])
        return reservation
...
def prepare_basket_for_order_submission(basket):
    ...
    for line in lines:
        ...
        reserved_quantity = _reservation_allocated_quantity(reservation)
    ...
    for reservation in reservations.values():
        ...
        allocated_quantity = _reservation_allocated_quantity(reservation)
        if locked_stockrecord and allocated_quantity:
            locked_stockrecord.cancel_allocation(allocated_quantity)
        reservation.delete()
